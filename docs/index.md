...
| Variable | Example Value | Description |
| --- | --- | --- |
| `SNC_DOMAIN` | `your.domain.org` | The domain that you want for your lab.  This will be part of your DNS setup |
| `SNC_NAMESERVER` | `10.11.11.10` | The IP address of your snc-host host. |
| `SNC_NETMASK` | `255.255.255.0` | The netmask of your router |
| `SNC_GATEWAY` | `10.11.11.1` | The IP address of your router |
| `INSTALL_HOST_IP` | `10.11.11.10` | The IP address of your snc-host host. |
| `INSTALL_ROOT` | `/usr/share/nginx/html/install` | The directory that will hold CentOS install images |
| `REPO_HOST` | `snc-host` | The snc-host hostname |
| `REPO_PATH` | `/usr/share/nginx/html/repos` | The directory that will hold an RPM repository mirror |
| `OKD4_SNC_PATH` | `~/okd4-lab` | The path from which we will build our OKD4 cluster |
...
| `LOCAL_REGISTRY` | `nexus.${SNC_DOMAIN}:5001` | The URL that we will use for our local mirror of the OKD registry images | 
| `LOCAL_REPOSITORY` | `origin` | The repository where the local OKD image mirror will be pushed |
| `LOCAL_SECRET_JSON` | `${OKD4_SNC_PATH}/pull-secret.json` | The path to the pull secret that we will need for mirroring OKD images |

When you have selected values for the variables.  Set them in the shell like this: 

    SNC_DOMAIN=your.domain.org
    SNC_NAMESERVER=10.11.11.10
    SNC_NETMASK=255.255.255.0
    SNC_GATEWAY=10.11.11.1
    ...
    OKD4_SNC_PATH=~/okd4-lab
    LOCAL_REGISTRY=nexus.${SNC_DOMAIN}:5001
...
Now, let's create a utility script that will persist these values for us:
    ...
    export SNC_DOMAIN=${SNC_DOMAIN}
    export SNC_NAMESERVER=${SNC_NAMESERVER}
    export SNC_NETMASK=${SNC_NETMASK}
    export SNC_GATEWAY=${SNC_GATEWAY}
    export REPO_HOST=${REPO_HOST}
    export INSTALL_HOST_IP=${INSTALL_HOST_IP}
    export INSTALL_ROOT=${INSTALL_ROOT}
    export REPO_URL=http://${REPO_HOST}.${SNC_DOMAIN}
    export INSTALL_URL=http://${INSTALL_HOST_IP}/install
    export REPO_PATH=${REPO_PATH}
    export OKD4_SNC_PATH=${OKD4_SNC_PATH}
...
Now is a good time to update and reboot the snc-host host:
...
__For the rest of this setup, unless otherwise specified, it is assumed that you are working from the snc-host Host.  You will need the environment variables that we just set up for some of the commands that you will be executing.__
